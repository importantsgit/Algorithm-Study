...
// 이 문제를 접근하는 방식
// - 양쪽의 값은 모두 들어갈 수 있음
// - 가운데 값은 양쪽을 비교해서 만약 양쪽의 최솟값보다 크다면 X


// 최솟값을 구하는 방법은 먼저 왼쪽부터 for문을 돌려 각 위치의 left 최소값을 구하고
// 다음으로 오른쪽 기준으로 양쪽을 비교하면서 min값을 갱신

import Foundation

func solution(_ a:[Int]) -> Int {
    if 1...2 ~= a.count { return a.count }
    
    var result = 2
    var leftMinValues = [Int](repeating: 0, count: a.count)
    leftMinValues[0] = a[0]
    
    for index in 1..<a.count-1 {
        leftMinValues[index] = min(a[index], leftMinValues[index-1])
    }
    
    var rightMin = a.last!
    
    for index in (1..<a.count-1).reversed() {
        if a[index] < leftMinValues[index-1] || a[index] < rightMin {
            result += 1
        }
        rightMin = min(rightMin, a[index])
    }
