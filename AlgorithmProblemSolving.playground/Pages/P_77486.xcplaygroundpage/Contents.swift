/*
 ...
 입출력 예
     referral(이름)    seller(참여)    amount    result
...
//    var result = [Int](repeating: 0, count: referral.count)
//    var map = [String: [String]]()
//    var sells = [String: [Int]]()
//    var indexs = [String: Int]()
//    
//    for (sel, amo) in zip(seller, amount) {
//        sells[sel, default: []].append(amo * 100)
//    }
//    
//    
//    for (offset, (enr, ref)) in zip(enroll, referral).enumerated() {
//        indexs[enr, default: 0] = offset
//        
//        map[(ref != "-" ? ref : "center"), default: []].append(enr)
//    }
//    
//    // 캐쉬 필요 X: 한 노드에 한 노드만 연결
//    // 기저사례 필요 X: 한 노드에 연결되어 있지 않으면 for 돌지 않음
//    func getValue(ref: String) -> Int {
//        
//        var returnValue = 0
//        
//        for enr in map[ref, default: []] {
//            let value = getValue(ref: enr)
//            returnValue += value
//            returnValue += Int(Double(value) * 0.1)
//            let getValue = value - Int(Double(value) * 0.1)
//            result[indexs[ref]!] += (getValue > 0 ? getValue : 0)
//        }
//        
//        for sells in sells[ref, default: []] {
//            returnValue += Int(Double(sells) * 0.1)
//            let getValue = sells - Int(Double(sells) * 0.1)
//            result[indexs[ref]!] += (getValue > 0 ? getValue : 0)
//        }
//
//        // 하위 판매원들의 합
...
//    }
//    
//    for ref in map["center", default: []] {
//        let _ = getValue(ref: ref)
//    }
//    
//    return result
//}

// Seller를 중심으로 For문을 돌렸을 때의 결과
...
func solution(_ enroll:[String], _ referral:[String], _ seller:[String], _ amount:[Int]) -> [Int] {
    ...
    for (offset, (enr, ref)) in zip(enroll, referral).enumerated() {
        indexs[enr, default: 0] = offset
        parents[enr, default: ""] = ref != "-" ? ref : "center"
    }
    ...
    for (sell, amo) in zip(seller, amount) {
        ...
        while ref != "center" {
            ...
            if toParent <= 0 {
                result[indexs[ref]!] += amo
                break
            }
            
            let addValue = (amo - toParent) > 0 ? (amo - toParent) : 0
            result[indexs[ref]!] += addValue
