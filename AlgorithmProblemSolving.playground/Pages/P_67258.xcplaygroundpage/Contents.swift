...
func solution(_ gems:[String]) -> [Int] {
    ...
    while end < gems.count {
        cert[gems[end], default: 0] += 1
        
        while start <= end && cert[gems[start], default:0] > 1 {
            cert[gems[start], default: 0] -= 1
            start += 1
        }
        
        if cert.count == gemsType.count
...
/*
 투 포인터를 이용하는 접근 방식은 맞았지만, start와 end를 어떻게 1+ 할 지 몰라 틀린 문제
 일단 start를 따로 while문을 돌려 +1씩 해줘야 함
 */
...
func solution2(_ gems:[String]) -> [Int] {
    ...
    while start <= end {
        if cert[gems[end], default: 0] <= 1 {
            // start를 빼야 함
            if cert[gems[start], default: 0] <= 1 {
